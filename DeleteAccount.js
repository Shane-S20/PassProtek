...
window.onload = function() {
    document.getElementById("DeleteAccountButton").onclick = () => {
        ...
        if(document.getElementById("masterPword").value == "" || document.getElementById("masterUname").value == "") {
            document.getElementById("deleteAccountSuccess").innerHTML = '<br><div class="alert alert-danger"style="width: 280px;text-align: center; margin: auto"><strong>Please Enter Your Master Password To Continue</strong></div>'
        } 
        else{
            ...
            const user_search = Data.get(encoded_user);

            user_search.onsuccess = () =>{
            var stored_input;
            var toggle = false;
            var keyRequest = Data.getAllKeys();
            keyRequest.onsuccess = () =>{
                for(i=0;i<10;i++){
                    if(encoded_user === keyRequest.result[i])
                    {
                        console.log("MATCH FOUND")
                        stored_input = user_search.result.Password;
                        toggle = true;
                        break
                    }
                }
                if(toggle = false){
                    ...
                    document.getElementById("deleteAccountSuccess").innerHTML = "INCORRECT CREDENTIALS, TRY AGAIN"
                }
                console.log("OUTSIDE LOOP")

                if(toggle = true){
                    console.log("IN PASSWORD TEST")
                    console.log("STORED INPUT "+stored_input)
                    console.log("ENTERED INPUT " +encoded_input)
                    if(encoded_input === stored_input){
